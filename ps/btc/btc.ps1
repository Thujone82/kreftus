...
.DESCRIPTION
    ...
    If LiveCoinWatch is unavailable, falls back to the CoinGecko Demo API (bundled key).
...
function Get-HttpStatusCode {
    ...
    if (-not $errorCode -and $exception.PSObject.Properties['Status'] -and $exception.Status) {
        try { $errorCode = [int]$exception.Status } catch { $errorCode = $null }
    }
    if (-not $errorCode -and $ErrorRecord.Exception.Message -match '(?:status code does not indicate success:\s*)(\d{3})') {
...
function Get-HistoricalData {
    param (
        ...
        [string]$Currency,
        [string]$CoinCode,
        [switch]$PreferCoinGecko
    )
    Write-Verbose "Getting historical API data..."
    if ($PreferCoinGecko) {
        return Get-CoinGeckoHistoricalData
    }
    ...
    try {
        ...
        if ($historicalResponse -and $historicalResponse.PSObject.Properties.Name -contains 'IsNetworkError' -and $historicalResponse.IsNetworkError) {
            $lcwErrorCode = $historicalResponse.ErrorCode
            if (Test-ApiAuthError -ErrorCode $lcwErrorCode) {
                return $historicalResponse
            }
            $lcwFailed = $true
        }
        elseif ($null -ne $historicalResponse.history -and $historicalResponse.history.Count -gt 0) {
            Write-Verbose "Historical API call completed. Count: $($historicalResponse.history.Count)"
        ...
        $lcwErrorCode = Get-HttpStatusCode $_
        if (Test-ApiAuthError -ErrorCode $lcwErrorCode) {
            return [PSCustomObject]@{ IsNetworkError = $true; ErrorCode = $lcwErrorCode }
        }
...
function Get-Sparkline {
    ...
    $sparkChars = [char[]](0x2581, 0x2582, 0x2583, 0x2584, 0x2585, 0x2586, 0x2587, 0x2588)
...
function Get-CoinGeckoApiData {
    try {
        ...
        $uri = "https://api.coingecko.com/api/v3/coins/markets?vs_currency=usd&ids=bitcoin&price_change_percentage=24h"
        $cgHeaders = @{ "x-cg-demo-api-key" = $coinGeckoDemoApiKey }
        $response = Invoke-RestMethod -Uri $uri -Method Get -Headers $cgHeaders -ErrorAction Stop
        if (-not $response -or $response.Count -eq 0) { return $null }
        $coin = $response[0]
        if ($null -eq $coin.current_price) { return $null }
        $athUsd = $null
        if ($coin.PSObject.Properties['ath'] -and $null -ne $coin.ath -and $coin.ath -gt 0) {
            $athUsd = [double]$coin.ath
        }
        return [PSCustomObject]@{
            rate           = [double]$coin.current_price
            volume         = if ($coin.PSObject.Properties['total_volume']) { [double]$coin.total_volume } else { 0 }
            cap            = if ($coin.PSObject.Properties['market_cap']) { [double]$coin.market_cap } else { 0 }
            allTimeHighUSD = $athUsd
            totalSupply    = if ($coin.PSObject.Properties['circulating_supply']) { $coin.circulating_supply } else { $null }
            liquidity      = $null
            delta          = [PSCustomObject]@{
                day       = if ($coin.PSObject.Properties['price_change_percentage_24h']) { [double]$coin.price_change_percentage_24h } else { 0 }
                hour      = $null
                week      = $null
                month     = $null
                year      = $null
...
function New-CoinGeckoFetchResult {
    $currentResponse = Get-CoinGeckoApiData
    if (-not $currentResponse -or $null -eq $currentResponse.rate) { return $null }
    $historicalStats = Get-CoinGeckoHistoricalData
    return [PSCustomObject]@{
        IsNetworkError  = $false
...
function Get-BitcoinApiData {
    ...
    if (-not $lcwFailed -and $currentResponse -and $currentResponse.PSObject.Properties.Name -contains 'IsNetworkError' -and $currentResponse.IsNetworkError) {
        $lcwErrorCode = $currentResponse.ErrorCode
        if (Test-ApiAuthError -ErrorCode $lcwErrorCode) {
            return [PSCustomObject]@{
                IsNetworkError  = $true
                ErrorCode       = $lcwErrorCode
    ...
    elseif (-not $currentResponse -or -not $currentResponse.rate) {
        $lcwFailed = $true
        $currentResponse = $null
    }
    ...
    if ($lcwFailed) {
        ...
        $coinGeckoResult = New-CoinGeckoFetchResult
    ...
    if ($null -ne $historicalStats -and $historicalStats.PSObject.Properties.Name -contains 'IsNetworkError' -and $historicalStats.IsNetworkError) {
        if (Test-ApiAuthError -ErrorCode $historicalStats.ErrorCode) {
            Write-Verbose "Historical auth error; continuing with current price only."
            $historicalStats = $null
        }
        else {
            Write-Verbose "Historical data unavailable; trying CoinGecko historical fallback..."
            $historicalStats = Get-CoinGeckoHistoricalData
        }
    }
    elseif ($null -eq $historicalStats) {
        $historicalStats = Get-CoinGeckoHistoricalData
    }
    return [PSCustomObject]@{
        IsNetworkError  = $false
...
$hasCurrentPrice = $fetchResult -and $fetchResult.CurrentResponse -and $fetchResult.CurrentResponse.PSObject.Properties['rate'] -and $null -ne $fetchResult.CurrentResponse.rate
if ((-not $hasCurrentPrice) -and ($null -eq $fetchResult -or -not (Test-ApiAuthError -ErrorCode $fetchResult.ErrorCode))) {
    Write-Verbose "Primary fetch failed; trying CoinGecko emergency fallback..."
    $coinGeckoResult = New-CoinGeckoFetchResult
    if ($coinGeckoResult) { $fetchResult = $coinGeckoResult }
}

try {
    $hasCurrentPrice = $fetchResult -and $fetchResult.CurrentResponse -and $fetchResult.CurrentResponse.PSObject.Properties['rate'] -and $null -ne $fetchResult.CurrentResponse.rate
    if ($null -eq $fetchResult -or $fetchResult.IsNetworkError -or -not $hasCurrentPrice) {
    ...
    if ($null -ne $currentResponse -and $currentResponse.PSObject.Properties.Name -contains "rate") {
        Write-Verbose "Successfully parsed current Bitcoin data."
        $bitcoinPrice = $currentResponse.rate
        $volume24h = if ($currentResponse.PSObject.Properties['volume']) { $currentResponse.volume } else { 0 }
        $marketCap = if ($currentResponse.PSObject.Properties['cap']) { $currentResponse.cap } else { 0 }
        $priceChange24hPercent_BTC_API = if ($currentResponse.delta -and $currentResponse.delta.PSObject.Properties['day']) { $currentResponse.delta.day } else { $null }
        $allTimeHighUSD = $null
        if ($currentResponse.PSObject.Properties['allTimeHighUSD'] -and $null -ne $currentResponse.allTimeHighUSD -and $currentResponse.allTimeHighUSD -gt 0) {
            $allTimeHighUSD = $currentResponse.allTimeHighUSD
        }
        $liquidity = if ($currentResponse.PSObject.Properties['liquidity']) { $currentResponse.liquidity } else { $null }
        $totalSupply = if ($currentResponse.PSObject.Properties['totalSupply']) { $currentResponse.totalSupply } else { $null }
        $deltaHourPct = if ($currentResponse.delta -and $currentResponse.delta.PSObject.Properties['hour']) { $currentResponse.delta.hour } else { $null }
        $deltaWeekPct = if ($currentResponse.delta -and $currentResponse.delta.PSObject.Properties['week']) { $currentResponse.delta.week } else { $null }
        $deltaMonthPct = if ($currentResponse.delta -and $currentResponse.delta.PSObject.Properties['month']) { $currentResponse.delta.month } else { $null }
        $deltaYearPct = if ($currentResponse.delta -and $currentResponse.delta.PSObject.Properties['year']) { $currentResponse.delta.year } else { $null }
        ...
        if ($null -ne $allTimeHighUSD -and $bitcoinPrice -ge ($allTimeHighUSD * (1 - $athProximityPercent))) { $bitcoinLineColorNameToUse = "Magenta" }
        ...
        function Get-LineWidth {
            ...
            # This is the actual character count of the line (the position of the last character + 1)
            ...
            Write-Verbose "Get-LineWidth: Label='$Label' (len=$($Label.Length)), ValueStartColumn=$ValueStartColumn, Padding=$paddingRequired, ValueString='$ValueString' (len=$($ValueString.Length)), ChangeString='$ChangeString' (len=$($ChangeString.Length)), PercentString='$PercentString' (len=$($PercentString.Length)), TotalWidth=$totalWidth"
            return $totalWidth
        }
        ...
        $btcChangeStr = if ($null -ne $priceDifference24h) { 
            ...
            $dollarSign = '$'
            " [$sign$dollarSign$formattedChangeAmount]"  # Format: " [+$123.45]" - space + bracket + sign + $ + amount + bracket
        } else { "" }
        Write-Verbose "Bitcoin ChangeString actual value: '$btcChangeStr' (length: $($btcChangeStr.Length))"
        $btcLineWidth = Get-LineWidth -Label "Bitcoin ($($currency)): " -ValueString $btcValueStr -ChangeString $btcChangeStr
        Write-Verbose "Bitcoin line width: $btcLineWidth, current maxLineWidth: $maxLineWidth"
        if ($btcLineWidth -gt $maxLineWidth) { $maxLineWidth = $btcLineWidth; Write-Verbose "Updated maxLineWidth to: $maxLineWidth" }
        ...
        if ($null -ne $mybtc -and $mybtc -gt 0) {
            ...
            $myBtcChangeStr = if ($null -ne $mybtcValueDifference24h) { 
                ...
                $dollarSign = '$'
                " [$sign$dollarSign$formattedChangeAmount]"  # Format: " [+$123.45]"
            } else { "" }
            Write-Verbose "My BTC ChangeString actual value: '$myBtcChangeStr' (length: $($myBtcChangeStr.Length))"
            $myBtcLineWidth = Get-LineWidth -Label "My BTC: " -ValueString $myBtcValueStr -ChangeString $myBtcChangeStr
            Write-Verbose "My BTC line width: $myBtcLineWidth, current maxLineWidth: $maxLineWidth"
            if ($myBtcLineWidth -gt $maxLineWidth) { $maxLineWidth = $myBtcLineWidth; Write-Verbose "Updated maxLineWidth to: $maxLineWidth" }
            ...
            if ($null -ne $myCOST -and $myCOST -gt 0) {
                ...
                Write-Verbose "Profit/Loss line width: $plLineWidth, current maxLineWidth: $maxLineWidth"
                if ($plLineWidth -gt $maxLineWidth) { $maxLineWidth = $plLineWidth; Write-Verbose "Updated maxLineWidth to: $maxLineWidth" }
        ...
        if ($null -ne $historicalStats -and -not $hasNetworkError) {
            ...
            if ($historicalStats.PSObject.Properties.Name -contains 'Sma1h' -and $historicalStats.Sma1h -gt 0) {
                ...
                $smaLineWidth = Get-LineWidth -Label "1H SMA: " -ValueString $smaValueStr -ValueStartColumn 16
                Write-Verbose "1H SMA line width: $smaLineWidth, current maxLineWidth: $maxLineWidth"
                if ($smaLineWidth -gt $maxLineWidth) { $maxLineWidth = $smaLineWidth; Write-Verbose "Updated maxLineWidth to: $maxLineWidth" }
            ...
            if ($historicalStats.PSObject.Properties.Name -contains 'Ago' -and $null -ne $historicalStats.Ago) {
                ...
                $agoLineWidth = Get-LineWidth -Label "24H Ago: " -ValueString $agoDisplay -ValueStartColumn 16
                Write-Verbose "24H Ago line width: $agoLineWidth, current maxLineWidth: $maxLineWidth"
                if ($agoLineWidth -gt $maxLineWidth) { $maxLineWidth = $agoLineWidth; Write-Verbose "Updated maxLineWidth to: $maxLineWidth" }
            ...
            if ($historicalStats.PSObject.Properties.Name -contains 'High' -and $null -ne $historicalStats.High) {
                ...
                $highLineWidth = Get-LineWidth -Label "24H High: " -ValueString $highDisplay -ValueStartColumn 16
                Write-Verbose "24H High line width: $highLineWidth, current maxLineWidth: $maxLineWidth"
                if ($highLineWidth -gt $maxLineWidth) { $maxLineWidth = $highLineWidth; Write-Verbose "Updated maxLineWidth to: $maxLineWidth" }
            ...
            if ($historicalStats.PSObject.Properties.Name -contains 'Low' -and $null -ne $historicalStats.Low) {
                ...
                $lowLineWidth = Get-LineWidth -Label "24H Low: " -ValueString $lowDisplay -ValueStartColumn 16
                Write-Verbose "24H Low line width: $lowLineWidth, current maxLineWidth: $maxLineWidth"
                if ($lowLineWidth -gt $maxLineWidth) { $maxLineWidth = $lowLineWidth; Write-Verbose "Updated maxLineWidth to: $maxLineWidth" }
        ...
        Write-Verbose "24H Volume line width: $volumeLineWidth, current maxLineWidth: $maxLineWidth"
        if ($volumeLineWidth -gt $maxLineWidth) { $maxLineWidth = $volumeLineWidth; Write-Verbose "Updated maxLineWidth to: $maxLineWidth" }
        ...
        Write-Verbose "Cap line width: $capLineWidth, current maxLineWidth: $maxLineWidth"
        if ($capLineWidth -gt $maxLineWidth) { $maxLineWidth = $capLineWidth; Write-Verbose "Updated maxLineWidth to: $maxLineWidth" }
        ...
        Write-Verbose "=== FINAL maxLineWidth before sparkline: $maxLineWidth ==="
        ...
        if ($null -ne $historicalStats -and -not $hasNetworkErrorForSparkline -and $historicalStats.PSObject.Properties.Name -contains 'History') {
            ...
            if ($historyData -and $historyData.Count -gt 0) {
                ...
                if ($hourlySamples.Count -ge 2) {
                    ...
                    # Calculate padding so the END of the sparkline aligns with the END of the widest line
                    # Total width needed: maxLineWidth (this is the 1-indexed column of the last character)
                    ...
                    Write-Verbose "Sparkline calculation: maxLineWidth=$maxLineWidth, historyLabel.Length=$($historyLabel.Length), sparklineLength=$sparklineLength"
                    $paddingRequired = $maxLineWidth - $historyLabel.Length - $sparklineLength
                    Write-Verbose "Calculated padding: $paddingRequired"
                    if ($paddingRequired -lt 0) { $paddingRequired = 0; Write-Verbose "Adjusted padding to 0 (was negative)" }
                    Write-Verbose "Final sparkline line width will be: $($historyLabel.Length) + $paddingRequired + $sparklineLength = $($historyLabel.Length + $paddingRequired + $sparklineLength)"
