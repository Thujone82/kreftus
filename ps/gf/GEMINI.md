...
- **Advanced Mode (Forecast import):** `-enableadvanced`/`-eadv [file]` enables Advanced mode into `%LOCALAPPDATA%\gf\gf.json` (`schemaVersion` 2). With a Forecast backup file, imports favorites + defaults and shows a colored import report. With no file, creates an empty profile (if needed) and opens the config modal so settings/locations can be built manually. `-disableadvanced`/`-dadv` confirms and removes it. `-load`/`-l N` starts on favorite slot N. `-config` opens a settings/favorites config modal (Advanced only) and exits without binding the last location or running weather fetch/geocode. Create/Edit Latitude accepts a `lat, lon` pair (e.g. `39.107, -77.190`) and skips the Longitude prompt when both are present. Setting **1** edits **Default Colors** (Primary `#00ff00`, Secondary `#00ced1`, Text `#00ced1` — Forecast accent defaults; hex pickers; titles use primary/secondary as truecolor, weather field text uses Text; `-config` chrome uses the same primary/secondary/text; per-location favorite colors still override when enabled). Setting **2** opens **Nerd Font glyphs** + title wrapper style (Off / Pill / Fire / Lean / Digital / Bars / Simple / Standard; default **off** / Simple; see **Nerd Font Glyphs Mode** below). Setting **14** edits the AirNow API key. Defaults cover Magic Hours, Irradiance (`-i` toggles), wildfire radius/filter, AQI (AirNow key → User env `AirNowAPI`), 24h clock, location-bar visibility, current mode, and per-location colors. CLI overrides win for the run. Location bar renders `▀ CustomName` with truecolor (active favorite fully highlighted; inactive show colored glyph only; with NF on, Powerline half-circle chips). **Tab** / **Shift+Tab** preview-wrap favorites and load **in-process** after an **800ms** settle (not on the control bar), hydrating from shared `weatherCache` when possible. If the location bar was **off** when Tabbing started, it is force-shown for the preview and **hidden again as soon as settle fires** (including wrap-back to the starting favorite). **Multi-session:** each window registers in `sessions` with heartbeat (~15s); per-location API leadership is the longest-running session whose `activeCacheKey` matches that location (not sticky — Tab-away promotes the next-oldest remaining viewer). Followers re-read cache; only the leader fetches NWS/AirNow/wildfire for that key. See **Weather Cache & Multi-Session Coordination** below. In TerseAlert, **A** toggles panes (Tab is reserved for favorites when Advanced is active). Section titles use favorite secondary for wrappers and primary for title text when per-location colors are enabled; otherwise Default Colors. With NF on, wrappers follow `nerdFontTitleStyle` (Pill/Fire/Lean/Digital/Bars/Simple/Standard) instead of `***`. Ignored import fields: UpdateAll, ShowRadar, AutoUpdate.
...
- Advanced boot shows progressive status (`Loading GF Advanced...`, cache restore, etc.) so the console is not blank during profile/cache I/O.
...
- **IRWIN Wildfires (Esri Living Atlas, current measurements):** `https://services9.arcgis.com/RHVPKKiFTONKtxq3/arcgis/rest/services/USA_Wildfires_v1/FeatureServer/0/query` — same spatial parameters. Fed straight from IRWIN, so it keeps updating when WFIGS publication stalls. Leaner schema: acreage is `DailyAcres` / `CalculatedAcres`, modified time is `ModifiedOnDateTime` (no `_dt` suffix), and there is **no** cost, fire behavior, `POOProtectingUnit`, or `IncidentShortDescription`. Does include impact counts (`ResidencesDestroyed`, `OtherStructuresDestroyed`, `Injuries`, `Fatalities`) for the **Losses:** line when any value is > 0.
...
### AQI Terminal Color Approximation
...
**Advanced profile:** `%LOCALAPPDATA%\gf\gf.json` (`schemaVersion` 2) — favorites, settings (including `defaultPrimaryColor` / `defaultSecondaryColor` / `defaultTextColor`, `useNerdFontGlyphs`, and `nerdFontTitleStyle`), shared `weatherCache`, and `sessions`. Edit with `-config` or import via `-eadv`.
...
**Nerd Font glyphs:** Advanced setting **2** (`useNerdFontGlyphs`, default `false`). Submenu selects **Off** or a title wrapper style (`nerdFontTitleStyle`: `pill` / `fire` / `lean` / `digital` / `bars` / `simple` / `standard`, default `simple`). Install **Cascadia Code NF** (or Cascadia Mono NF / CaskaydiaCove) and set it as the terminal Font face. Enabling detects installed Cascadia/Caskaydia NF families and warns if none are found. See **Nerd Font Glyphs Mode** below.

### Nerd Font Glyphs Mode

Opt-in Advanced feature controlled by `settings.useNerdFontGlyphs` (config setting **2**). When enabled, `Get-GfGlyph` prefers the Nerd Font registry over emoji/ASCII fallbacks for the whole session. Title chrome uses `settings.nerdFontTitleStyle` via `Write-GfNfWrappedTitle`.
...
| Section titles | Style from `nerdFontTitleStyle`: **Pill** `E0B6`/`E0B4`, **Fire** `E0C2`/`E0C0`, **Lean** `E0BA`/`E0BC`, **Digital** `E0C7`/`E0C6` (title BG = secondary cap color; Fire/Lean/Digital also pad with `█`), **Bars** `=== Title ===`, **Simple** `──┤`/`├──` (no fill), or **Standard** `*** Title ***`; body in primary; titles render **bold** (ANSI SGR) |
| Location bar | Powerline half-circles `E0B6` / `E0B4` when NF framing is active |
| `-config` | Setting **2** submenu with live style samples; setting-row icons use **secondary** color; labels/values use Default Text; bool toggles `F205`/`F204`; chrome truecolor from Default Colors; NF/title-style saves use locked profile patch |
...
**Implementation notes:** Glyph tables live in `Initialize-GfGlyphTables` (`gfEmojiGlyphs` / `gfNerdGlyphs`). Titles: `Write-GfThemedSectionTitle` → `Write-GfNfWrappedTitle` / `Get-GfTitleWrapperSpec` (bold via `Write-GfTrueColorText -Bold`). Config UI: `Invoke-GfConfigNerdFontGlyphs`, `Write-GfConfigBanner`, `Write-GfConfigSectionHeader`, `Write-GfConfigSettingLine`, `Write-GfConfigNfIcon`, `Get-GfConfigBoolLabel`. Settings persistence: `Update-GfAdvancedProfileFields` → `Invoke-GfAdvancedProfileLocked`. Flag + style loaded in `Initialize-GfAdvancedMode` and refreshed each `-config` redraw. Advanced boot status defaults to `Loading GF Advanced...`.
...
- **Nerd Font Glyphs (Advanced setting 2):** Opt-in Cascadia Code NF icon set for weather UI and `-config`; selectable bold title wrappers (Pill/Fire/Lean/Digital/Bars/Simple/Standard); see **Nerd Font Glyphs Mode**.
...
# Hide wildfires at or below 1 acre from display and counts
.\gf.ps1 "Reno, NV" -nsf
...
### Recent Enhancements (v2.7)
- **Title wrapper styles completed:** Setting **2** submenu order **Pill** / **Fire** / **Lean** / **Digital** / **Bars** (`=== Title ===`) / **Simple** (`──┤`/`├──`) / **Standard** (`*** Title ***`); Fire/Lean/Digital keep `█` pads; titles render **bold**.
- **Config polish:** Setting-line NF icons use Default **secondary** color; labels/values stay on Default **text**. Locked `Update-GfAdvancedProfileFields` for NF/title-style (and other config settings) so concurrent session heartbeats cannot clobber saves; profile load retries torn reads; config modal falls back to in-memory profile instead of aborting with “Advanced profile missing.”
- **Boot status:** Advanced launch message shortened to `Loading GF Advanced...` (no location suffix).

### Recent Enhancements (v2.6)
- **Nerd Font glyphs (Advanced setting 2):** Opt-in `useNerdFontGlyphs` (default off) with `nerdFontTitleStyle` (`pill` / `fire` / `lean` / `digital` / `bars` / `simple` / `standard`). Cascadia Code NF / CaskaydiaCove PUA icons for conditions, 28-step moon, alerts/wildfire/precip/trends/wind, compact °F/°C, Magic/Irradiance/Updated prefixes, selectable title wrappers, Powerline location chips, and `-config` icons/toggles. Enable warns if no Cascadia/Caskaydia NF family is installed; terminal Font face must be set to Cascadia Code NF (or Mono NF). Default Colors wire into `-config` chrome (primary/secondary/text). Magic Hours sun glyph is Yellow/Blue by line. PUA display width forced to 1 cell in NF mode.
...
- **Solar Irradiance Display:** Clear-sky GHI (W/m²) at current time plus peak at solar noon with time, after Sunset in white, shown in both full and terse modes

### Solar Irradiance (v2.1)

The solar irradiance feature displays estimated clear-sky global horizontal irradiance (GHI) in W/m² at the current time for the location, plus the peak GHI at the location's solar noon and the time of solar noon. It appears after the Sunset line in the current conditions section (full mode, terse mode, and hourly/daily/observations).
