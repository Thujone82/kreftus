...
- **Change (trade confirmation)**: Optional line after Market Rate when the offer mirrors the last opposite ledger entry—**Sell** when offer BTC matches last Buy (delta in USD); **Buy** when offer USD matches last Sell (delta in BTC). Green/red/white on the value; label stays white. **Market Rate** appends an uncolored bracketed USD delta vs the prior opposite entry's `BTC(USD)` when the same conditions apply. Ledger lookup uses `readAllLedgerEntries` (current + archives); match tolerances: 1 satoshi (`0.00000001`) BTC (integer satoshi comparison), `0.01` USD
...
#### Data flow
- **Price providers**: `fetchCurrentPriceData` tries `fetchLiveCoinWatchCurrentPrice` first; on provider/network failure (not 401/403), falls back to `fetchCoinGeckoCurrentPrice` (`GET /api/v3/simple/price?ids=bitcoin&...` with `x-cg-demo-api-key`). `getHistoricalData` tries `fetchLiveCoinWatchHistoricalData` first; on failure falls back to `fetchCoinGeckoHistoricalData` (`GET /api/v3/coins/bitcoin/market_chart?vs_currency=usd&days=1` with demo key), mapping `prices[][]` to `HistoryResponse`.
