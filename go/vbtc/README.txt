...
- Real-time Bitcoin market data (Price, 24h Change, High, Low, Volatility [velocity], Volume).
...
- **Volatility line:** Displays as "Volatility: X.XX% [N]" where the bracketed value is the velocity (see Tips).
...
- Run with `-verbose` or `-v` to print velocity calculation details to stderr (TotalChange, 1HourDeltaTotal, 24H High/Low, range, Volatility, hourlyAvg, multiplier, and resulting velocity)
