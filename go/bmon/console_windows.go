...
// writeConsole writes using WriteConsoleW to ensure Unicode rendering.
func writeConsole(s string) {
    ...
    var written uint32
    writeConsoleW.Call(h, uintptr(unsafe.Pointer(&buf[0])), uintptr(len(buf)-1), uintptr(unsafe.Pointer(&written)), 0)
}
