...
- `render_test.go` — solved overlay title is **{Difficulty} Solved!**; uses **Perfect Finish!** at 0 mistakes and colon-aligns Time / Errors otherwise; **New Best Completion!** / **New Fastest Completion!** only when the run is the new record; Best vs Fastest label/value; Stats box top border centers the title; Shift+digit symbols map to 1–9; NumLock+Shift keypad nav keys map to 1–9 and stay pencil-active through Windows' fake Shift-up; pause progress bar and Pencil Marks / Errors line; replay celebrate shows the solution and digit hue-shift wraps.
