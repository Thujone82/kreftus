"""Prepare png2ico.ico for PyInstaller / Windows shell display.

Vista-style ICO files may store sizes as PNG-compressed payloads.
PyInstaller copies those bytes into the EXE, but Explorer often falls back
to the default bootloader icon when RT_ICON entries are PNG.
This script keeps BMP/DIB entries only. png2ico stores 256×256 as PNG and
smaller sizes as BMP; this rewrite drops the PNG so Explorer can show the
embedded EXE icon.
"""
