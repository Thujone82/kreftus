"""Prepare thermo.ico for PyInstaller / Windows shell display.

Vista-style ICO files may store large sizes as PNG-compressed payloads.
PyInstaller copies those bytes into the EXE, but Explorer often falls back
to the default bootloader icon when the largest RT_ICON entry is PNG.
This script rewrites the ICO using BMP/DIB entries only.
"""
