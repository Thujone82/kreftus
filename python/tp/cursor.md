...
- **BLE recovery:** Prompt before enabling Bluetooth when the radio is off (`ble_radio.py` + `BluetoothPermissionModal`); invalidate per-device BLE cache on each fetch failure; auto power-cycle after whole-fleet failure **or** a device fail streak ≥ 2 (full-fleet polls only), then retry; if still stuck (streak ≥ 3), run elevated `reset_bluetooth.ps1` (PnP adapter reset) — ask in-app only when UAC would actually prompt; auto-run when UAC is Never notify / already elevated; 90 s radio cooldown, 15 min stack-reset cooldown, 5 min re-prompt cooldown after decline
