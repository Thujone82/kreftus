...
- **Monitoring:** 5 rows per device; green/yellow device name by freshness; fetch arrows show BLE step (cyan connecting / green once linked — history or sync read / yellow passive); sequential BLE fetch (one device at a time, 60 s timeout); optional multi-column layout (**C**; auto-fits terminal width on load); **T** / **Shift+T** cycle dashboard sparkline window forward / reverse (Less: 24H → 72H → 4H; More: 24H → 36H → 72H → 90M → 4H → 8H → 12H)
...
- **CSV logging:** Optional append-only log; default `tp_log.csv`; written and fsynced after each device in a poll cycle (not deferred to cycle end); 72h preload on mount/resume; renaming log file in Options renames on disk (overwrite prompt if target exists)
...
- **BLE recovery:** Prompt before enabling Bluetooth when the radio is off (`ble_radio.py` + `BluetoothPermissionModal`); invalidate per-device BLE cache on each fetch failure; auto power-cycle after whole-fleet failure **or** a device fail streak ≥ 2 (full-fleet polls only), then retry; if still stuck (streak ≥ 3), auto-run elevated `reset_bluetooth.ps1` (PnP adapter reset) — no in-app Y/N; Windows UAC is the only consent UI when configured to notify; 90 s radio cooldown, 15 min stack-reset cooldown, 5 min re-prompt cooldown after decline (enable-Bluetooth only)
...
Connect path caches resolved `BLEDevice` for 120 s and retries with extended scan (10 s) after a quick scan (5 s) miss. A short (~1 s) settle pause runs between devices (no cancelled mid-gap scanner prefetch — that could wedge WinRT). Failed fetches invalidate that MAC's cache. When Bluetooth is disabled, `is_bluetooth_radio_disabled()` detects the state and `ensure_bluetooth_enabled_for_polling()` shows `BluetoothPermissionModal` (Y/N) before `enable_bluetooth_radio()`. On `BleakBluetoothNotAvailableReason.POWERED_OFF` or scan/read errors, the same enable prompt is used. After a whole-fleet failure **or** a per-device fail streak ≥ 2 on a **full-fleet** poll, `restart_bluetooth_radio()` power-cycles the adapter automatically (no prompt) and retries the failing devices. If the streak still reaches ≥ 3, TemPy runs elevated `reset_bluetooth.ps1` (PnP stack reset) and retries again — no in-app Y/N; Windows UAC is the only consent UI when the OS is set to notify, and with Never notify the reset elevates silently so monitoring recovers without interruption. Stack reset process wait is capped at 120 s (kill on timeout) so a wedged PnP reset cannot freeze TemPy overnight. Minute retries / partial subsets skip radio and stack recovery (power-cycling mid-Retry wedges WinRT). WinRT radio ops are bounded by a 15s timeout. CSV preload reads a sized tail window (not a full-file binary search) so large out-of-order logs stay fast at startup.
...
Append after **each device** within a fetch cycle (including partial retries), with flush+fsync so a mid-cycle freeze keeps prior units. Incremental mode may append multiple rows per device. UTF-8, `\n` line endings. Preload last 72h on mount; seeds `last_updated` and fetch status per device.
